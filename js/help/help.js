function help(text = null) {
  ...
  if (!text) {
    ...
    help.innerHTML = `
    <input class="help-input" type="text" placeholder="How do I start a game?" onkeyup="event.preventDefault(), clearTimeout(helpKeyUp), helpKeyUp = setTimeout(() => (this.value.trim() && help(this.value.trim())), 400)">
    `;
    swal({
      title: "Help center",
    ...
    fetch("js/help/help.json").then(response => response.json()).then(helpList => {
      let responses = [];
      let content = "";
